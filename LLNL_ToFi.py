...
from utils import (nl_UM_TZ, np_UM_TZ, np_LM, OUTPUT_PATH, OUTFILE_FILT_PREFIX)
...
def main(argv):
    ...
    [slowness_perturbation, v_1D] = model.reparam(
    ...
    for ilyr in range(1, nl + 1):
        ...
        for ip in range(my_ib, my_ie + 1):
            ...
            # calculate current row index in the part of the model vector for the current layer
            cri = row_i[ip] - offset - 1

            # Tomographically filtered model
            my_m_prime[cri] = my_m_prime[cri] + R_ij[ip] * \
                slowness_perturbation[l_index][c_index]

        # Now collect all contributions on the master proc
        comm.Reduce([my_m_prime, MPI.DOUBLE], [
                    m_prime, MPI.DOUBLE], op=MPI.SUM, root=0)
        ...
        if myrank == 0:
            ...
            # dv = du*v_1D^2 => dv/v_1D = du*v_1D
            m_prime = m_prime * -v_1D[ilyr - 1]

            # Now output the filtered model
            utils.write_layer(
                ilyr, m_prime, radii[ilyr - 1]["avg"], lon, gc_lat, OUTFILE_FILT_PREFIX)
